...
function App() {
  ...
  // ── All export/data functions unchanged ──────────────────────────────────
  ...
  const exportMonth = async () => {
    try {
      ...
      const report = data.map(d => {
        const noteMoney = notesData.filter(n => n.student_id === d.student_id && n.date === d.date).reduce((sum, n) => sum + (n.money || 0), 0);
  ...
  const handleAttendance = async (studentId) => {
    ...
    try {
      ...
      if (data.length === 0) {
        const { error: insertError } = await supabase.from('attendance').insert([{ student_id: studentId, date: today, status: true }]);
        if (insertError) { alert('Lỗi insert: ' + insertError.message); return; }
      } else {
  ...
  const handleSaveNote = async () => {
    ...
    try {
      await supabase.from('notes').insert([{ student_id: selectedStudentId, content: noteContent, money: Number(noteMoney) || 0, date: selectedDate }]);
  ...
  const exportRange = async () => {
    try {
      ...
      const noteMap = {};
      (noteData || []).forEach(n => { noteMap[`${n.student_id}_${n.date}`] = { content: n.content, money: n.money || 0 }; });
      ...
      dates.forEach(date => {
        studentsData
          .filter(student => selectedExportStudents.length === 0 || selectedExportStudents.includes(String(student.id)))
          .forEach(student => {
            ...
            report.push({ "Ngày": date, "Học sinh": student.name, "Trạng thái": attendanceMap[key] ? "Có mặt" : "Vắng", "Ghi chú": noteMap[key]?.content || "", "Tiền phát sinh": extraMoney > 0 ? extraMoney.toLocaleString('vi-VN') : "", "Tiền buổi học": sessionMoney > 0 ? sessionMoney.toLocaleString('vi-VN') : "", "Tổng cộng": totalMoney > 0 ? totalMoney.toLocaleString('vi-VN') : "" });
          });
      });
      ...
      studentsData.forEach(student => {
        if (selectedExportStudents.length === 0 || selectedExportStudents.includes(String(student.id))) {
          ...
          const totalNotesMoney = (noteDataFromMonthStart || []).filter(n => n.student_id === student.id).reduce((sum, n) => sum + (n.money || 0), 0);
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
          ...
          --shadow-md: 0 6px 24px rgba(108,99,255,.12);
        ...
        .header-title {
          ...
          background: linear-gradient(90deg, #fff 0%, #fde68a 40%, #fbcfe8 80%, #fff 100%);
        ...
        .date-range-input:focus { border-color: var(--brand-1); }
        .arrow-sep {
          font-weight: 900; font-size: 15px;
          color: var(--brand-1); flex-shrink: 0;
        }
        ...
        .student-btn.absent:hover {
          border-color: var(--brand-3);
          transform: translateY(-3px);
          box-shadow: 0 8px 24px rgba(67,197,158,.18);
        }
        .student-btn.present {
        ...
        .student-btn.absent .student-avatar {
          background: linear-gradient(135deg, #E0E7FF 0%, #C7D2FE 100%);
          color: #6C63FF;
        }
        .student-btn.present .student-avatar {
        ...
        .student-btn.absent .student-status-pill {
          background: #FEF3C7; color: #92400E;
        }
        .student-btn.present .student-status-pill {
        ...
        @media (max-width: 768px) {
          .delete-btn { opacity: 1 !important; transform: scale(1) !important; }
        }
        ...
        .btn-add:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(108,99,255,.45); }
        ...
        .archived-card {
          ...
          min-width: 100px;
          box-shadow: 0 3px 10px rgba(249,168,212,.2);
        ...
        @media (max-width: 640px) {
          ...
          .header-logout { top: 12px; right: 12px; padding: 6px 12px; font-size: 11px; }
          .main { padding: 16px 12px 40px; }
          .stats-grid { gap: 10px; }
          .stat-value { font-size: 30px; }
          .stat-card { padding: 16px 12px; }
          .student-grid { grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 10px; }
      ...
      <main className="main">
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card green">
            ...
            <span className="stat-value">{attendanceList.length}</span>
          </div>
          <div className="stat-card red">
            <span className="stat-label">Vắng</span>
            <span className="stat-value">{students.length - attendanceList.length}</span>
          </div>
        </div>
        ...
        <div className="panel" style={{ marginBottom: '24px' }}>
          <div className="panel-header">
            ...
            <div className="att-controls">
              <input
                type="date" className="date-range-input"
                value={fromDate}
              ...
              <div ref={filterRef} className="filter-wrap">
                ...
                {showSearchBox && (
                  <div className="filter-dropdown">
                    <input
                      type="text" className="filter-search"
                    ...
                    <label className="filter-label" style={{ fontWeight: 800, marginBottom: 8 }}>
                      <input
                        ...
                        checked={selectAll}
                        onChange={() => {
                          if (selectAll) { setSelectedExportStudents([]); }
                          else { setSelectedExportStudents(students.map(s => String(s.id))); }
                      ...
                      .map(s => (
                        <label key={s.id} className="filter-label">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedExportStudents([...selectedExportStudents, String(s.id)]);
                              } else {
                                setSelectedExportStudents(selectedExportStudents.filter(id => id !== String(s.id)));
                                setSelectAll(false);
                              }
                            }}
                          />
              ...
              <button className="btn-export" onClick={exportRange}>
                📊 Xuất file
              </button>
            </div>
          </div>
                  ...
                  <div key={s.id} className="student-card">
                    ...
                    >
                      ...
                      <span className="student-status-pill">
                        {isLoading ? '...' : isPresent ? 'Có mặt' : 'Vắng'}
                      </span>
                    </button>
                    <span
              ...
              {showArchived && (
                <div className="archived-panel">
                  ...
                  {archivedStudents.length === 0 ? (
                    <p style={{ textAlign: 'center', color: '#d8b4fe', fontSize: '13px', fontWeight: 600 }}>
          ...
          <div className="panel panel-note">
            ...
            <div className="panel-body" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <textarea
                ...
                placeholder="Lời nhắn cho phụ huynh..."
              />
              <input
                type="text"
                inputMode="numeric"
                ...
                onChange={(e) => {
                  ...
                  if (raw === '' || /^\d+$/.test(raw)) setNoteMoney(raw);
                }}
              ...
              <button
                ...
                style={{
                  ...
                  color: '#fff',
                  boxShadow: '0 6px 18px rgba(168,85,247,.35)'
                }}
              >
                LƯU GHI CHÚ 💖
              </button>
